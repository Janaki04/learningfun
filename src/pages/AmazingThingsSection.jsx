...
export default function AmazingThingsSection() {
  ...
  useEffect(() => {
    if (currentIndex === 0) {
      timeoutRef.current = setTimeout(() => {
        ...
        setCurrentIndex(loopCards.length - 2); 
      }, 500);
  ...
  return (
    <section className="bg-white py-7 px-2 md:px-4 font-sans overflow-hidden select-none">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <div className="h-1.5 w-24 bg-[#3A5AFF] mx-auto mt-3 rounded-full"></div>
        </div>
          ...
          {originalCards.map((_, idx) => (
            <button
              ...
              className={`rounded-full transition-all duration-300 ${
                activeMapIndex === idx 
                  ? 'w-3 h-3 bg-[#3A5AFF] hover:bg-[#FFC43B]' 
